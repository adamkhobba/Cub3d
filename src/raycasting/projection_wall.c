...
/*   Updated: 2025/03/26 01:56:19 by akhobba          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void render_projection_walls(t_ray *rays, int num_rays)
{
    ...
    while (i < num_rays - 1)
    {
		...
		WALL_STRIP_W, (HEIGHT- wall_strip_h) / 2, data->map->color_c);
		fillrect((t_point){i * WALL_STRIP_W, (HEIGHT + wall_strip_h) / 2},
		WALL_STRIP_W, (HEIGHT - wall_strip_h) / 2, data->map->color_f);
        double start_y = (HEIGHT/ 2) - (wall_strip_h / 2);
        ...
        double tex_x;
        if (rays[i].was_hit_vertical)
            tex_x = fmod(rays[i].wall_hit.y, CUB_SIZE);
        else
            tex_x = fmod(rays[i].wall_hit.x, CUB_SIZE);
        ...
        for (int y = 0; y < HEIGHT; y++)
        {
            ...
            {
                double tex_y = (offset_y * vertical_scale);
...
// 				- (wall_strip_h / 2)}, WALL_STRIP_W, wall_strip_h, 0xFFFFFF);
// 		else
// 			fillrect((t_point){i * WALL_STRIP_W, (data->mlx.win_height / 2)
